function [ para_opt, X_train, Y_opt, Y_train ] = Single_Track_XXVII_Adaptive_Limit_Memory_opt_Joint_UKF_11Dof (vxj, vyj, wzj, wFj, wRj, epsij, eyj, sj, deltaj, Tj, dt, curvaturej)
...
epsij=epsij( Index(1):window+Index(1) );
eyj=eyj( Index(1):window+Index(1) );
sj=sj( Index(1):window+Index(1) );
% Xj=Xj( Index(1):window+Index(1) );
% Yj=Yj( Index(1):window+Index(1) );
deltaj=deltaj( Index(1):window+Index(1) );
Tj=Tj( Index(1):window+Index(1) );
curvaturej=curvaturej( Index(1):window+Index(1) );
...
Omit=tail/dt;
lth=length(Tj);
M=window0/((window0-13)+dT); % data split into 2 parts, first half for training
...
%%
lFR = 0.57;
%          m   Iz   lF    IwF   IwR   h   B   C   D  st  Th   To    bias  
para0 = [  22  1.2 0.30  0.040 0.040 0.1 1.1 1.1 1.4 35.7 204  0.16 0.13  0.70 0.75];
%          Iz         lf            h                  B               C      D       E                    Sh                 Sv                     scl              Cd                  Ix               Iy              Kf       Kr       Cf       Cr 
para_lb=[  20  1.0 0.28  0.030 0.030 0.05 0.9 0.9 1.2 25.0 195  0.10 0.10   0.3  0.3];
para_ub=[  26  1.4 0.32  0.050 0.060 0.2 1.5 1.5 1.6 40.0 215  0.2 0.15   2.0  2.0];
...
x0(1:num_states)=[ vxj(1) vyj(1) wzj(1) wFj(1) wRj(1) epsij(1) eyj(1) sj(1)]'
...
%   Detailed explanation goes here
   ...
   Y=[ vxj vyj wzj wFj wRj epsij eyj sj ]; 
   Ye=Y;
   Yeb=Ye;
...
%     Xk = Sgm_x1(9, jj);
%     Yk = Sgm_x1(10,jj);
    deltak = m_Vehicle_kSteering * deltaj(k);
    Tk = m_Vehicle_kThrottle * Tj(k);
    curvaturek=curvaturej(k);
...
%     muR = tire_D*sin( tire_C*atan( tire_B*sER - tire_E*(tire_B*sER - atan(sER) ) ) ) + tire_Sv;
    ...
    dot_epsi = (wzk - (vxk * cos(epsik) - vyk * sin(epsik)) / (1 - curvaturek * eyk) * curvaturek);
    epsik = epsik + bias_epsij;
    dot_ey = (vxk * sin(epsik) + vyk * cos(epsik));
    dot_s = (vxk * cos(epsik) - vyk * sin(epsik)) / (1 - curvaturek * eyk);
%     dot_X = vxk * cos(epsik) - vyk * sin(epsik);
%     dot_Y = vxk * sin(epsik) + vyk * cos(epsik);
   ...
   Sgm_x2(5,jj)=Sgm_x1(5,jj)+dot_wR*dt;
   Sgm_x2(6,jj)=Sgm_x1(6,jj)+dot_epsi*dt;
   Sgm_x2(7,jj)=Sgm_x1(7,jj)+dot_ey*dt;
   Sgm_x2(8,jj)=Sgm_x1(8,jj)+dot_s*dt;
...
%    Svj=Sgm_x1_aux(18);
 ...
 %% simulated results
   ...
   Xs(N,1:num_states)=[ vxj(N) vyj(N) wzj(N) wFj(N) wRj(N) epsij(N) eyj(N) sj(N)];  %Xs(N,11:12)=[ PitchAngle(N) RollAngle(N) ];
...
%     dot_Y = vxk * sin(epsik) + vyk * cos(epsik);
   ...
   Xs(k+1,5)=Xs(k,5)+dot_wR*dt; 
   Xs(k+1,6)=Xs(k,6)+dot_epsi*dt;
   Xs(k+1,7)=Xs(k,7)+dot_ey*dt;
   Xs(k+1,8)=Xs(k,8)+dot_s*dt;
...
%     fprintf('The cost is %e\n', J);
    ...
    para_opt=[ mj Izj lFj IwFj IwRj hj Bj Cj Dj kSteerj kThrottlej kTorquej bias_epsij mu1j mu2j];
...
t=0:dt:(dt*(N-1));
    ...
    legend('True epsi','Estimated epsi');
    subplot(4,2,7);
    plot(t,Y(1:N,7),'b',t,Y_train(1:N,7),'r');
    legend('True ey','Estimated ey');
    subplot(4,2,8);
...
%     legend('True yaw angle','Estimated yaw angle');
    ...
    subplot(6,2,3);
    plot(t,X(1:N,num_states+3)*scales(3));legend('lF');    
    subplot(6,2,4);
    plot(t,X(1:N,num_states+4)*scales(4));legend('IwF');
    subplot(6,2,5);
    plot(t,X(1:N,num_states+5)*scales(5));legend('IwR');
    subplot(6,2,6);
    plot(t,X(1:N,num_states+6)*scales(6));legend('h'); 
    subplot(6,2,7);
