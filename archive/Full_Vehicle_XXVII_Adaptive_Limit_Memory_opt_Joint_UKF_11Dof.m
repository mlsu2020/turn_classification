function [ para_opt, J, X_train, Y_opt, Y_train ] = Full_Vehicle_XXVII_Adaptive_Limit_Memory_opt_Joint_UKF_11Dof (XI, YI, YawAngle2, Vx, Vy, r, RollAngle, PitchAngle, steering, steering2, w_lf, w_lr, w_rf, w_rr, dt)
...
Area=0.138; %(reference area =0.138)
...
Omit=tail/dt;
lth=length(r);
M=window0/((window0-12)+dT); % data split into 2 parts, first half for training
...
para0 = [  1.2,          0.4,               0.1,              1.3,       1.3,              1.2,         -0.526797954883659,  -0.0514327220075633,  -0.0454297657839960,       10,        0.00000000000,       1.0,              1.2,         1000,          1000,                1000            18];
%          Iz         lf        h        B       C       D       E       Sh      Sv     scl      Cd     Ix      Iy        Kf       Kr       Cf       Cr 
para_lb=[  1.0       0.3        0.05       0.9       0.9      0.9      -10     -0.2    -0.1    8      0    0.2     0.8     90         90     0      15   ];   %lower boundry
para_ub=[  1.4       0.5        0.2       1.3      1.3      1.3     10       0.2     0.1    15       0.5    2.0    2.0     10000    10000     1000     24  ];   %upper boundry
...
%    Xs(k+1,:)=Xs(k,:)+[ dot_Vx dot_Vy dot_r dot_XI dot_YI Xs(k,3) dot_Zs dot_theta dot_phi Xs(k,7) Xs(k,8) Xs(k,9)]*dt+bar_q_buf(1:12,N)';  
    ...
    para_opt=[ Iz lf h B C D E Sh Sv scl Cd Ix Iy Kf Kr Cf ms ];
