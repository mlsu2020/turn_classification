function [ para_opt, J, X_train, Y_opt, Y_train ] = My_XXVII_Adaptive_Limit_Memory_opt_Joint_UKF_11Dof (Tj, Vxj, Xj, Sj, Vyj, Yj, dt)
...
x0(1:num_states)=[ Tj(1) Vxj(1) Xj(1) Vyj(1) Yj(1)]'
Y=[Vxj; Xj; Yj];  % use column vector
...
%    f_rry=mu_rry*TireForce_z(4);
   ...
   dot_V= Sgm_x1(1,jj)/m;  %no aerodynamics
...
%    
   Sgm_x2(:,jj)=Sgm_x1(:,jj)+[ 0; dot_V; dot_X; dot_Vyj; dot_Yj; zeros(num_params,1)]*dt+Sgm_w(:,jj)*sqrt(dt);
   Sgm_x2s(:,jj)=Sgm_x1(:,jj)+[ 0; dot_V; dot_X; dot_Vyj; dot_Yj; zeros(num_params,1)]*dt;
    ...
    Cdz=[zeros(num_meas,1) [1;0;0] [0;1;0] zeros(num_meas,1) [0;0;1] zeros(num_meas,num_params)];
%     size(Sgm_x2)
%     size(Sgm_v)
%     size(Cdz)
...
%    
   Sgm_x2(:,jj)=Sgm_x1(:,jj)+[ 0; dot_V; dot_X; dot_Vyj; dot_Yj; zeros(num_params,1)]*dt+Sgm_w(:,jj)*sqrt(dt);
   Sgm_x2b(:,jj)=Sgm_x1(:,jj)+[ 0; dot_V; dot_X; dot_Vyj; dot_Yj; zeros(num_params,1)]*dt+Sgm_wb(:,jj)*sqrt(dt);
   Sgm_x2s(:,jj)=Sgm_x1(:,jj)+[ 0; dot_V; dot_X; dot_Vyj; dot_Yj; zeros(num_params,1)]*dt;
...
%    Cr=X_aux(17);  
 ...
 %% simulated results
   ...
   Xs(N,1:num_states)=[ Tj(N) Vxj(N) Xj(N) Vyj(N) Yj(N)];  %Xs(N,11:12)=[ PitchAngle(N) RollAngle(N) ];
%    delta=scl*steering2;
...
%    dot_phi=( -Wa^2*Kf*Xs(k,12)/2-Wa^2*Cf*Xs(k,9)/2-Wb^2*Kr*Xs(k,12)/2-Wb^2*Cr*Xs(k,9)/2+(F_slfy+F_srfy)*(h-r_F)+(F_slry+F_srry)*(h-r_R) )/Ix;   
   ...
   Xs(k+1,:)=Xs(k,:)+[ 0 dot_V dot_X dot_Vyj dot_Yj]*dt;
...
t=0:dt:(dt*(N-1));
    figure;
    subplot(3,1,1);
    plot(t,Y(1:N,1),'b',t,Y_train(1:N,2),'r');
    legend('True T','Estimated T');     
    subplot(3,1,2);
    plot(t,Y(1:N,2),'b',t,Y_train(1:N,3),'r');
    legend('True V','Estimated V');  
    subplot(3,1,3);
    plot(t,Y(1:N,3),'b',t,Y_train(1:N,5),'r');
...
%     legend('True yaw angle','Estimated yaw angle');
    ...
    subplot(3,2,2);
    plot(t,X(1:N,num_states+2)*scales(2));legend('S');    
%     subplot(3,2,3);
...
%     plot(t,X(1:N,23)*scales(11));legend('Air Coef'); 
    ...
    figure;
    subplot(3,1,1);
    plot(t,Y(N:lth-1,1),'b',t,Y_opt(N:lth-1,2),'r');
    legend('True T','Simulated T');
    subplot(3,1,2);
    plot(t,Y(N:lth-1,2),'b',t,Y_opt(N:lth-1,3),'r');
    legend('True V','Simulated V');
    subplot(3,1,3);
    plot(t,Y(N:lth-1,3),'b',t,Y_opt(N:lth-1,5),'r');
